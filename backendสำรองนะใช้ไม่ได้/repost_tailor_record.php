...
if (!empty($start_date) && !empty($end_date)) {
    $condition = "WHERE tailor_record.created_at BETWEEN '$start_date' AND '$end_date'";
} elseif (!empty($start_date)) {
    $condition = "WHERE tailor_record.created_at >= '$start_date'";
} elseif (!empty($end_date)) {
    $condition = "WHERE tailor_record.created_at <= '$end_date'";
}
?>
<!DOCTYPE html>
<html lang="th">
        ...
        <form method="GET" action="">
            <div class="row mb-3">
                ...
                <div class="col-md-4 d-flex align-items-end">
                    ...
                    <a href="repost_tailor_record.php" class="btn btn-secondary ms-2">ล้างค่า</a>
                </div>
            </div>
        </form>
        ...
        <table id="memberTable" class="table table-striped table-bordered">
            <thead>
                <tr>
                    ...
                    <th scope="col">วัน/เดือน/ปี ที่ส่งผ้า</th>
                </tr>
            </thead>
            <tbody>
                <?php
                $sql = "SELECT *, SUM(quantity) AS sum_quantity FROM `tailor_record` 
                        INNER JOIN tailor ON tailor_record.tailor_id = tailor.id
                        INNER JOIN product ON tailor_record.product_id = product.product_id $condition
                        GROUP BY tailor_id 
