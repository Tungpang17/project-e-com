...
<div class="container">
  ...
  <h3>รายงานสินค้าขายดี 5 อันดับ  <button type="button" class="btn btn-light" onclick="print();" id="hid"> 
...
<?php
$sql="SELECT * ,sum(`sall_detail`.`amont`) AS `sumqty` FROM `product`
right JOIN `sall_detail` ON `product`.`product_id`=`sall_detail`.`product_id`
GROUP BY `sall_detail`.`product_id`
